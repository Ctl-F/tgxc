...
#define TGX_DISPATCH(table, sys) goto *table[((Instruction*)((sys).Memory.memory_begin + (sys).PU.gp32[REG_IP].full))->opcode]
...
/* 0x0004 */        TGX_ADDR(MOVR64_R64),        TGX_ADDR(NOP),            TGX_ADDR(NOP),            TGX_ADDR(MOVTBL_R64_IC8),
/* 0x0008 */        TGX_ADDR(MOVTBL_R64_IR8),    TGX_ADDR(MOVR64_TBL_IC8), TGX_ADDR(MOVR64_TBL_IR8), TGX_ADDR(MOVR32_R64H),
...
//#error "Not Implemented"
    ...
    TGX_CASE(MOVTBL_R64_IC8):
    TGX_PROFILE_CALL(MOVTBL_R64_IC8, 0x0007);
//#error "Not Implemented"
    TGX_PROFILE_END(MOVTBL_R64_IC8, 0x0007);
    TGX_NEXT_INSTR(*sys);
    TGX_DISPATCH(_jTable, *sys);

    TGX_CASE(MOVTBL_R64_IR8):
    TGX_PROFILE_CALL(MOVTBL_R64_IR8, 0x0008);
//#error "Not Implemented"
    TGX_PROFILE_END(MOVTBL_R64_IR8, 0x0008);
    TGX_NEXT_INSTR(*sys);
    TGX_DISPATCH(_jTable, *sys);

    TGX_CASE(MOVR64_TBL_IC8):
    TGX_PROFILE_CALL(MOVR64_TBL_IC8, 0x0009);
//#error "Not Implemented"
    TGX_PROFILE_END(MOVR64_TBL_IC8, 0x0009);
    TGX_NEXT_INSTR(*sys);
    TGX_DISPATCH(_jTable, *sys);

    TGX_CASE(MOVR64_TBL_IR8):
    TGX_PROFILE_CALL(MOVR64_TBL_IR8, 0x000A);
//#error "Not Implemented"
    TGX_PROFILE_END(MOVR64_TBL_IR8, 0x000A);
    TGX_NEXT_INSTR(*sys);
    TGX_DISPATCH(_jTable, *sys);
